...
def motion_metrics(plan_trajectory, prediction_trajectories, ground_truth_trajectories, weights):
    ...
    plannerADE = torch.count_nonzero(prediction_distance).item()/(prediction_distance.numel()*2-torch.count_nonzero(prediction_distance).item())
    ...
    plannerFDE = predictorFDE.clone()
    plannerFDE[plannerFDE<=2.7] = 0
    plannerFDE[plannerFDE>2.7] = 1
    plannerFDE = torch.mean(plannerFDE)

    predictorFDE = torch.mean(predictorFDE)

    return plannerADE, plannerFDE.item(), predictorADE.item(), predictorFDE.item()
