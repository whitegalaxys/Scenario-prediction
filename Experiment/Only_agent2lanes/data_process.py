...
class DataProcess(object):
    ...
    def map_process(self, traj, timestep, type=None):
        ...
        '''
        vectorized_map = np.zeros(shape=(6, 100, 17))
        ...
        if agent_type != 2:
            ...
            for neighbor_lane, start in neighbor_lane_ids.items():
                candidate = depth_first_search(neighbor_lane, self.lanes, dist=lane_polylines[neighbor_lane][start:].shape[0], threshold=200)
                ref_lanes.extend(candidate)
        ...
        for i, s_lane in enumerate(ref_lanes):
            ...
            # scale the lane
            vectorized_map[i] = cache_lane[np.linspace(0, added_points, num=100, endpoint=False, dtype=np.int)]
    ...
    def ego_process(self, sdc_id, timestep, tracks):
        ego_states = np.zeros(shape=(self.hist_len, 8))
        ...
        self.current_xyh = np.array((tracks[sdc_id].states[timestep].center_x, tracks[sdc_id].states[timestep].center_y,
                                     tracks[sdc_id].states[timestep].heading))
            ...
            for i, neighbor_state in enumerate(neighbor_states):
                if neighbor_state.valid:
                    neighbors_states[added_num, i] = np.array([neighbor_state.center_x, neighbor_state.center_y, neighbor_state.heading, neighbor_state.velocity_x,
                                                               neighbor_state.velocity_y, neighbor_state.length, neighbor_state.width, neighbor_state.height, neighbor_type])
            added_num += 1
    ...
    def normalize_data(self, ego, neighbors, map_lanes, map_crosswalks, ref_line, ground_truth, viz=True):
