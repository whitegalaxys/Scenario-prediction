...
class AgentEncoder(nn.Module):
    def __init__(self):
        ...
        # 改进点1：使用BiLSTM
        self.motion = nn.LSTM(8, 128, 2, batch_first=True, bidirectional=True)
    ...
    def forward(self, inputs):
        ...
        # 改进点1：使用BiLSTM
        traj, (final_hidden_state, final_cell_state) = self.motion(inputs[:, :, :8])

        # 改进点2：加上了注意力机制

        # hidden: [batch_size, hidden_size(=128) * num_directions(=2), n_layer(=2)]
        hidden = final_hidden_state.view(-1, 128 * 2, 2)
        hidden = hidden[:,:,-1].unsqueeze(-1)

        # [batch_size, hidden_size * num_directions(=2), n_step] * [batch_size, n_step, 1] = [batch_size, n_hidden * num_directions(=2), 1]
        attn_weights = torch.bmm(traj, hidden).squeeze(2)
        soft_attn_weights = F.softmax(attn_weights, 1)

        # [batch_size, n_layer * num_directions(=2)]
        output = torch.bmm(traj.transpose(1, 2), soft_attn_weights.unsqueeze(2)).squeeze(-1)
...
class Agent2Map(nn.Module):
    ...
    def forward(self, actor, lanes, crosswalks, mask):
        ...
        for i in range(len(crosswalks_actor)):
            crosswalks_actor[i] = torch.ones_like(crosswalks_actor[i])
            lanes_actor[i] = torch.ones_like(lanes_actor[i])
